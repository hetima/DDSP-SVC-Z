...
def exec_main_diff(ddsp_model, diff_model, input_path, output_path):
    ...
    args.extend(["-ddsp", ddsp_model])
    args.extend(["-diff", diff_model])
    args.extend(["-o", output_path])
    # 追加の引数
    args.extend(["-kstep", "300"])
    subprocess.run(args)
...
def select_project():
    ...
    for f in os.listdir(dir_path):
        if os.path.isdir(os.path.join(dir_path, f)):
            if os.path.exists(os.path.join(dir_path, f, DDSP_MODEL_DIR)) and os.path.exists(os.path.join(dir_path, f, DIFF_MODEL_DIR)):
                dirs.append(f)
    slct = inquirer.select(message="Select model:", choices=dirs, multiselect=False).execute()
    return slct

def models_in_project(project_path):
    ddsp_path = os.path.join(project_path, DDSP_MODEL_DIR)
...
def do_main():
    ...
    ddsp_path, diff_path = models_in_project(project_path)
    if not ddsp_path:
        print("ddsp model not found")
        exit(0)
    if not diff_path:
