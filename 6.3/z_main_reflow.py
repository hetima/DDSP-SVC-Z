    ...
    for f in os.listdir(dir_path):
        if os.path.isdir(os.path.join(dir_path, f)):
            if os.path.exists(os.path.join(dir_path, f, DIFF_MODEL_DIR)):
                dirs.append(f)
    slct = questionary.select("Select model:", choices=dirs).ask()
    return slct
...
def do_main():
    parser = argparse.ArgumentParser(description="DDSP-SVC-Z z_main_reflow")
    ...
    print(output_path)
    exec_main_reflow(diff_path, input_path, output_path)
